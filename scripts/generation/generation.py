...
def generate_biome_collection(game, scene, size, tile_size, id, config, boss, spawn=True):
...
def generate_room(game, biome):
    ...
    for index, pos in enumerate(path):
        ...
        room[pos].link(*generate_biome_collection(game, room[pos], game.size, 32, biome, get_biome_type(previous, pos, next), next==None, pos != (0, 0)).tilemaps.values())
    return room